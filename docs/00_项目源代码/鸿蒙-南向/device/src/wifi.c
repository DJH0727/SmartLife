...
assoc_req.ssid[strlen(WIFI_SSID)] = '\0';  // 确保ssid是以\0结尾的字符串

/* 设置加密方式 */
assoc_req.auth = HI_WIFI_SECURITY_WPA2PSK;

/* 设置热点密码 */
rc = memcpy_s(assoc_req.key, HI_WIFI_MAX_KEY_LEN + 1, WIFI_PASSWORD, strlen(WIFI_PASSWORD));
if (rc != EOK) {
    return -1;
}
assoc_req.key[strlen(WIFI_PASSWORD)] = '\0';  // 确保密码是以\0结尾的字符串
